...
console.log(iphone.isShiny());  // 여기서 this는 iphone을 가리킨다.
var Gadget = function(){};

Gadget.isShiny = function(){
  // return 'you bet';
  return this;  // this가 무엇을 가르키는지 알아보기 위해 this를 리턴.
};
...
var iphone = new Gadget();
iphone.setPrice(500); // 값을 셋팅하고,
console.log(iphone.price);  // 가져온다.
// 인스턴스 메서드를 스태틱 메서드와 같은 방법으로 호출하면 동작하지 않는다.
// 스태틱 메서드 역시 인스턴스인 iphone객체를 사용해 호출하면 동작하지 않는다.
...
공개/비공개 스태틱 프로퍼티는 특정 인스턴스에 한저오디지 않는 메서드와 데이터를 담을 수 있고, 인스턴스 별로 매번 재생성되지 않아 편리하다.
