...
### 어원
샌드박스는 영어 그대로 해석하자면 "모래상자"라는 뜻이다. 아이들이 놀이터에서 놀 수 있는 공간을 의미한다. 모래 상자 안에는 당연히 모래가 있겠고 아이들이 모래를 가지고 놀다보면 그 공간은 상당히 더러워 질 수가 있다. 어원에 담겨 있듯이 샌드박스란 **정해진 공간 안에서 형식 없이 마음껏 자유자재로 놀아라** 는 뜻을 가지고 있다.
